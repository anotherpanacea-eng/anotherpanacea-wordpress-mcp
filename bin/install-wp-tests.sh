...
if [[ $WP_VERSION =~ ^[0-9]+\.[0-9]+$ ]]; then
	...
	if [[ $WP_VERSION =~ [0-9]+\.[0-9]+\.[0] ]]; then
		# version x.x.0 means the first release of the major version
		# so skip .0 and just use the major version
...
install_wp() {
	...
	if [[ $WP_VERSION == 'nightly' || $WP_VERSION == 'trunk' ]]; then
		...
		unzip -q $TMPDIR/wordpress.zip -d $TMPDIR/wordpress-extract
		mv $TMPDIR/wordpress-extract/wordpress/* $WP_CORE_DIR
...
install_db() {
	...
	elif [ $(which mariadb-admin) ]; then
		mariadb-admin create $DB_NAME --user="$DB_USER" --password="$DB_PASS"$EXTRA
	else
		echo "Neither mysqladmin nor mariadb-admin found. Cannot create database."
		exit 1
	fi
}
